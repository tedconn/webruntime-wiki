...
But before creating the components, we need our application to specify where our components live. The LWC convention for doing this is adding an `lwc.config.json` to our package root (adjacent to the `package.json`):
